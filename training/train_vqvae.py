...
def main():
    ...
    if args.gpu is not None:
        # User specified specific GPU(s)
        gpu_ids = [int(g.strip()) for g in args.gpu.split(',')]

        # Validate GPU IDs
        for gpu_id in gpu_ids:
            if gpu_id >= available_gpus:
                raise ValueError(f'GPU {gpu_id} not available. Available GPUs: 0-{available_gpus-1}')

        # Set CUDA_VISIBLE_DEVICES to restrict to specified GPUs
        os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
        ngpus = len(gpu_ids)
        print(f'Using specified GPU(s): {args.gpu}')
    else:
        # Use specified number of GPUs or all available
        if args.ngpus is not None:
            ngpus = min(args.ngpus, available_gpus)
            if args.ngpus > available_gpus:
                print(f'Warning: requested {args.ngpus} GPUs but only {available_gpus} available')
        else:
            ngpus = available_gpus

    print(f'Executing script on {ngpus} GPUs (of {available_gpus} available)')
...
def main_worker(rank, size, args_in):
    ...
    if ckpt is not None:
        model, hp = load_model(
            ...
            freeze_model=False,
            cond_types=tuple(),
            foveated_loss=args.foveated_loss,
            foveated_sigma=args.foveated_sigma,
        )
    else:
    ...
    if is_root:
        ...
        if args.foveated_loss:
            print(f'Foveated loss ENABLED with sigma={args.foveated_sigma}')
...
def make_grid_video(real: torch.Tensor, recon: torch.Tensor) -> torch.Tensor:
    """Create side-by-side video grid for tensorboard.

    Args:
        real: (b, c, t, h, w) in [-0.5, 0.5]
        recon: (b, c, t, h, w) in [-0.5, 0.5]

    Returns:
        combined: (b, t, c, h, w*2) in [0, 1] for tensorboard add_video
    """
    real = (real.detach().cpu() + 0.5).clamp(0, 1)
    recon = (recon.detach().cpu() + 0.5).clamp(0, 1)
    # Concatenate along width dimension (side by side: real | recon)
    combined = torch.cat([real, recon], dim=4)  # (b, c, t, h, w*2)
    # Permute to (b, t, c, h, w) for add_video
...
def save_reconstructions(test_loader, model, iteration, writer, device, is_root, inputs_fn):
    ...
    # === Video logging (new) ===
    if is_root:
        # Create side-by-side video: real | reconstruction
        video_grid = make_grid_video(images, images_recon)  # (b, t, c, h, w*2)
        # Log first 4 videos to tensorboard
        writer.add_video('reconstruction/video', video_grid[:4], iteration, fps=8)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu', type=str, default=None,
                        help='specific GPU(s) to use, e.g., "0" or "0,1" (overrides --ngpus)')

    parser.add_argument('--cfg', type=str, default=None,
                        help='model configs, ignored if ckpt is provided')

    # Foveated loss options
    parser.add_argument('--foveated_loss', action='store_true',
                        help='Use foveated (center-weighted) loss function')
    parser.add_argument('--foveated_sigma', type=float, default=0.5,
                        help='Sigma for foveated Gaussian weighting (smaller = sharper falloff, default: 0.5)')
