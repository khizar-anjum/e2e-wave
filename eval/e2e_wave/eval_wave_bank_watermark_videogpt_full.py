    ...
    for batch in tqdm(val_loader, desc="Eval batches"):
        ...
        for snr_db in snr_points:
            ...
            for b, (rx_flat, targets, counts) in enumerate(seq_results):
                ...
                min_len = min(real_frames.shape[0], recon_frames.shape[0])
                real_frames = real_frames[:min_len]
                recon_frames = recon_frames[:min_len]

                mse_val = F.mse_loss(recon_frames, real_frames)
                psnr_val = _psnr_from_mse(mse_val).item()
                ssim_val = ssim_metric(recon_frames, real_frames).item()
                ssim_metric.reset()

                if save_videos and save_this_snr:
                    clip_counters[snr_key] += 1
                    if save_every <= 1 or (clip_counters[snr_key] - 1) % save_every == 0:
                        clip_idx = clip_indices[b]
                        stem = Path(paths[b]).stem
                        clip_tag = (
                            f"{stem}_clip{int(clip_idx):06d}" if clip_idx is not None else stem
                        )
                        recon_name = f"{clip_tag}_snr{snr_db:.1f}dB.mp4"
                        _write_video(recon_dir / recon_name, recon, VIS_FPS)
                        orig_key = (paths[b], clip_idx)
                        if orig_key not in saved_originals:
                            orig_name = f"{clip_tag}_orig.mp4"
                            _write_video(orig_dir / orig_name, real, VIS_FPS)
                            saved_originals.add(orig_key)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate waveform bank over SNR sweep.")
    parser.add_argument("--video_dir", required=True, help="Root video dataset folder.")
    ...
    parser.add_argument("--max_clips", type=int, default=VAL_MAX_LENGTH)
    ...
    parser.add_argument("--output_dir", default="", help="Folder for reconstructed/original videos.")
    parser.add_argument("--waveform_len", type=int, required=True, help="Waveform length for bank.")
    parser.add_argument("--save_videos", action="store_true", help="Write recon/original videos.")
    parser.add_argument(
        "--save_snr",
        default="",
        help="Comma-separated SNR list for video saving (e.g., '0,10,20').",
    )
    parser.add_argument(
        "--save_every",
        type=int,
        default=1,
        help="Save every Nth clip per SNR (1 = all).",
    )
    ...
    if "input_shape" in ckpt.get("hp", {}):
        try:
            t_in, h_in, _ = ckpt["hp"]["input_shape"]
    ...
    val_loader = DataLoader(
        EvalVideoDataset(
            ...
            max_length=args.max_clips if args.max_clips > 0 else None,
        ),
    ...
    csv_path = Path(args.output_csv) if args.output_csv else bank_dir / f"eval_{bank_dir.name}_snr_sweep.csv"
    csv_path.parent.mkdir(parents=True, exist_ok=True)
    output_dir = Path(args.output_dir) if args.output_dir else bank_dir / "eval_videos"
    save_snr = []
    if args.save_snr:
        for item in args.save_snr.split(","):
            item = item.strip()
            if item:
                save_snr.append(float(item))
    fieldnames = ["snr_db", "video_path", "psnr", "ssim"]
