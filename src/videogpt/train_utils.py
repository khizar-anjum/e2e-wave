...
def load_model(
    ...
    model, hp = config_model(
        configs_str='', **ckpt['hp'], cond_types=cond_types)
        # foveated_loss=foveated_loss, foveated_sigma=foveated_sigma)  # hp will be overwritten
    model = model.to(device=device)

    model.load_state_dict(ckpt['state_dict'], strict=False)  # strict=False to allow new foveated params
